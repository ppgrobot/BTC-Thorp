#!/bin/bash
# Deploy ETH Price Collector and Trading Bot Lambdas
...
# ==========================================
# Deploy ETH Trading Bot Lambda
# ==========================================
TRADER_NAME="ETHTradingBot"
...
if aws lambda get-function --function-name $TRADER_NAME --region $REGION 2>/dev/null; then
    ...
    aws lambda create-function \
        ...
        --runtime python3.12 \
        --role $LAMBDA_ROLE \
        --handler eth_lambda_function.lambda_handler \
        --zip-file fileb://$ZIP_FILE \
        --timeout 60 \
        --memory-size 256 \
        --region $REGION \
...
TRADER_RULE_EARLY="ETHTradingBot-Minute30"
...
TRADER_RULE_LATE="ETHTradingBot-Minute45"
