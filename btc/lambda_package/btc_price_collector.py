...
# Include shorter windows for dynamic trading window (5-15 min before settlement)
VOL_WINDOWS = [5, 7, 10, 12, 15, 30, 60, 90, 120]
...
def lambda_handler(event, context):
    """
    Main Lambda handler - runs every minute but collects prices every 10 seconds.

    Collects 6 price samples per minute (every 10 seconds) to provide
    more granular volatility data for the trading bot.
    """
    try:
        print(f"BTC Price Collector starting at {datetime.utcnow().isoformat()}")

        dynamodb = boto3.resource('dynamodb')
        prices_collected = []

        # Collect prices every 10 seconds for ~50 seconds
        # This gives us 6 samples per minute
        for i in range(6):
            ...
            if price:
                ...
                prices_collected.append({'price': price, 'timestamp': timestamp})
                print(f"[{i+1}/6] BTC Price: ${price:,.2f} at {timestamp.strftime('%H:%M:%S')}")

            # Wait 10 seconds before next collection (except on last iteration)
            if i < 5:
                time.sleep(10)
        ...
        if not prices_collected:
            return {
                'statusCode': 500,
                'body': json.dumps({'error': 'Could not fetch any BTC prices'})
            }

        # Calculate volatility for each window using all collected data
        vol_metrics = {}
        for window in VOL_WINDOWS:
            ...
            if vol:
                print(f"  {window}m: std={vol['std_dev']:.4f}%, samples={vol['sample_count']}")
        ...
        return {
            'statusCode': 200,
            'body': json.dumps({
                'prices_collected': len(prices_collected),
                'last_price': prices_collected[-1]['price'],
                'timestamp': prices_collected[-1]['timestamp'].isoformat(),
