#!/bin/bash
# Setup API Gateway for BTC Volatility API with Bearer token auth

set -e

REGION="us-east-1"
ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
LAMBDA_NAME="BTCVolatilityAPI"
API_NAME="BTCVolatilityAPI"
...
echo "=========================================="
echo "Setting up BTC Volatility API"
echo "=========================================="
...
cd lambda_package
rm -f ../btc_lambda.zip
zip -r ../btc_lambda.zip . -x "*.pyc" -x "__pycache__/*" -x "*.so" > /dev/null
cd ..
...
if aws lambda get-function --function-name $LAMBDA_NAME --region $REGION 2>/dev/null; then
    ...
    aws lambda create-function \
        ...
        --runtime python3.12 \
        --role $LAMBDA_ROLE \
        --handler btc_volatility_api.lambda_handler \
        --zip-file fileb://btc_lambda.zip \
        --timeout 30 \
        --memory-size 128 \
...
echo "$BEARER_TOKEN" > .api_token
echo "Token also saved to .api_token (gitignored)"
