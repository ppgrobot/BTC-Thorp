#!/bin/bash
# Deploy SOL Price Collector and Trading Bot Lambdas
...
ZIP_FILE="sol_lambda.zip"

echo "=========================================="
echo "Deploying SOL Lambdas"
echo "=========================================="
...
# ==========================================
# Deploy SOL Price Collector Lambda
# ==========================================
COLLECTOR_NAME="SOLPriceCollector"
...
if aws lambda get-function --function-name $COLLECTOR_NAME --region $REGION 2>/dev/null; then
    ...
    aws lambda create-function \
        ...
        --runtime python3.12 \
        --role $LAMBDA_ROLE \
        --handler sol_price_collector.lambda_handler \
        --zip-file fileb://$ZIP_FILE \
        --timeout 30 \
        --memory-size 128 \
        --region $REGION
...
COLLECTOR_RULE="SOLPriceCollector-EveryMinute"
...
# ==========================================
# Deploy SOL Trading Bot Lambda
# ==========================================
TRADER_NAME="SOLTradingBot"
...
if aws lambda get-function --function-name $TRADER_NAME --region $REGION 2>/dev/null; then
    ...
    aws lambda create-function \
        ...
        --runtime python3.12 \
        --role $LAMBDA_ROLE \
        --handler sol_lambda_function.lambda_handler \
        --zip-file fileb://$ZIP_FILE \
        --timeout 60 \
        --memory-size 256 \
        --region $REGION \
...
TRADER_RULE_EARLY="SOLTradingBot-Minute30"
...
TRADER_RULE_LATE="SOLTradingBot-Minute45"
