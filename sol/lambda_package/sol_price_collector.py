"""
SOL Price Collector Lambda

Runs every minute to:
1. Fetch SOL price from Coinbase
...
def lambda_handler(event, context):
    ...
    """
    try:
        print(f"SOL Price Collector starting at {datetime.utcnow().isoformat()}")

        # Get current SOL price
        price = get_coinbase_sol_price()
        if not price:
            return {
                'statusCode': 500,
