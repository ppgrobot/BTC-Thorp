"""
Kalshi SOL (Solana) Hourly NO Contract Bot

Strategy: Buy NO contracts on strikes above current SOL price when our
...
Position Sizing:
- Shares Kelly allocation with BTC/ETH/XRP bots via DynamoDB tracking
- Combined exposure across all assets capped at MAX_KELLY_FRACTION (0.10 for SOL)
...
MIN_VOLATILITY_PCT = 0.20  # Higher floor for SOL's typical volatility
...
MAX_VOLATILITY_PCT = 15.0  # Higher threshold for SOL

# Kalshi event series for SOL hourly
SOL_SERIES = "KXSOLD"

# DynamoDB table for volatility data
VOL_TABLE = "SOLPriceHistory"

# DynamoDB table for trade logs
TRADE_LOG_TABLE = "SOLTradeLog"
...
def calculate_model_probability(sol_price, strike_price, vol_std_pct, minutes_to_settlement):
    """
    Calculate our model's probability that SOL stays below strike.
    """
    ...
    price_diff_pct = (strike_price - sol_price) / sol_price * 100
    ...
    print(f"  Strike ${strike_price:,.2f} is {price_diff_pct:.3f}% above current ${sol_price:,.2f}")
...
def get_coinbase_sol_price():
    """Fetch current SOL price from Coinbase API."""
    try:
        ...
        print(f"Coinbase SOL price: ${price:,.2f}")
        return price

    except Exception as e:
        print(f"Error fetching Coinbase SOL price: {e}")
        return None


def get_next_hour_event_ticker():
    """Generate the Kalshi event ticker for the NEXT hour's SOL contract."""
    et_time = get_et_time()
    ...
    event_ticker = f"{SOL_SERIES}-{year}{month}{day}{hour}"
    print(f"Next hour event ticker: {event_ticker} (settles at {hour}:00 ET)")
    return event_ticker


def get_sol_markets(event_ticker):
    """Fetch all markets for a SOL hourly event from Kalshi."""
    try:
        ...
        print(f"Error fetching SOL markets: {e}")
        traceback.print_exc()
        return []


def find_target_strike(markets, sol_price, min_bps=MIN_BPS_ABOVE):
    """Find the first strike that is at least min_bps basis points above current SOL price."""
    min_strike = sol_price * (1 + min_bps / 10000)
    print(f"Looking for first strike >= ${min_strike:,.2f} ({min_bps}bps above ${sol_price:,.2f})")
...
def log_trade(trade_data):
    ...
    try:
        ...
        item = {
            ...
            'sol_price': Decimal(str(trade_data.get('sol_price', 0))),
...
def execute_no_trade(ticker, count, price, trade_context=None):
    ...
    try:
        ...
        if trade_context:
            trade_log_data = {
                **result,
                'sol_price': trade_context.get('sol_price'),
...
def lambda_handler(event, context):
    """
    Main Lambda handler - SOL Hourly NO Strategy
    """
    try:
        ...
        # Step 3: Get SOL price and target contract
        print("\n=== Step 3: SOL Price & Contract ===")
        sol_price = get_coinbase_sol_price()
        if not sol_price:
            return {
                'statusCode': 500,
                'body': json.dumps({'error': 'Could not fetch SOL price'})
            }

        event_ticker = get_next_hour_event_ticker()
        markets = get_sol_markets(event_ticker)
        if not markets:
            return {
                'statusCode': 200,
        ...
        target_market = find_target_strike(markets, sol_price)
        ...
        if order_result:
            update_hour_position(hour_key, 'sol', kelly['kelly_fraction'])
