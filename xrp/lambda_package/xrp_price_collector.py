"""
XRP Price Collector Lambda

Runs every minute to:
1. Fetch XRP price from Coinbase
...
def store_price(dynamodb, price, timestamp):
    ...
    print(f"Stored price: ${price:,.4f} at {timestamp.isoformat()}")
...
def lambda_handler(event, context):
    ...
    """
    try:
        print(f"XRP Price Collector starting at {datetime.utcnow().isoformat()}")

        # Get current XRP price
        price = get_coinbase_xrp_price()
        if not price:
            return {
                'statusCode': 500,
